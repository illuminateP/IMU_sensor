서버 종료 방법(Ctrl+C)을 추가하고, 명령어와 코드가 눈에 잘 띄도록 박스(Code Block) 처리를 적용하여 작성한 `README.md`입니다.

아래 내용을 복사하여 파일에 저장하시면 됩니다. 요청하신 대로 `*` 기호는 전혀 사용하지 않았습니다.

-----

# IMU Sensor Data Logger (HTTPS)

이 프로젝트는 스마트폰의 가속도 및 자이로스코프 센서 데이터를 실시간으로 PC로 전송하여 저장하고 시각화합니다.
